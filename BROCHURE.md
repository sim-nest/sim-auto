...
Automotive work needs names for vehicles, diagnostic lanes, byte-level fault frames, service operations, parts lookup, supplier orders, shop-side access, and replayable diagnostic sessions without leaking private shop data into the runtime. This repository gives SIM a shared vocabulary for those pieces, with synthetic fixture discipline baked into the tests.

## Why you will be glad

- Vehicle-facing experiments can use a common language instead of ad hoc labels.
- Diagnostic frames, modeled ECU replies, catalog entries, and supplier orders become inspectable records while shop capabilities stay explicit, so access and effects are easy to review.
